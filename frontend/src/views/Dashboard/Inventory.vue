    ...
    <div class="container mx-auto mt-5 relative">
                ...
                <div class="flex flex-row w-full">
                    ...
                    <div class="w-6/12 flex justify-end">
                        <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                            @click="addPopup = false" class="cursor-pointer">
                ...
                <form class="mt-10 flex flex-col gap-5" @submit.prevent="addItem">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <select v-model="item.category_id"
                            class="appearance-none border rounded bg-white w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    ...
                    <button type="submit" @submit="addItem"
                        class="bg-gradient-to-br from-green-400 to-green-600 block px-4 py-2  text-white rounded-full h-full flex items-center justify-center">Add</button>
                </form>
            </div>
        ...
        <div class="w-full p-10 rounded-md border bg-gray-50">
            <div class="flex flex-col md:flex-row w-full items-center gap-4">
                ...
                <span @click="addPopup = true"
                    class="cursor-pointer bg-gradient-to-br from-green-400 to-green-600 block px-4 py-2  text-white rounded-full h-full flex items-center justify-center"
                    v-if="user.id == getUser().id">+</span>
            </div>
                ...
                <div v-for="category in categories" :key="category.id">
                    <div class="flex flex-col gap-2 mt-10" v-if="hasItems(category.id)">
                        ...
                        <div class="flex flex-row gap-5 items-center justify-start">
                            <div class="min-w-[250px] bg-white rounded-xl border"
                                v-for="item in itemsByCategory(category.id)" v-bind:key="item.id">
                                <div class="p-3">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div class="flex flex-row p-3 gap-3">
                                            <a :href="'https://www.facebook.com/sharer/sharer.php?u=' + currentLink"
                                                class="px-4 bg-blue-200 text-blue-500 rounded-full text-xs max-w-max py-2">
                                                FB
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div class="bg-green-200 text-green-500 text-center rounded-b-xl py-2 text-xs uppercase cursor-pointer hover:bg-green-300 transition hover:text-green-600"
                                    v-if="user.id != getUser().id" @click="claimItem(item.id)" :id="item.id">
                                    Claim
                                </div>
                            </div>
                        </div>
