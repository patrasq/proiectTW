    ...
    <header>
        <div class="container mx-auto flex flex-row py-10">
            ...
            <div class="flex flex-row gap-5 items-center w-4/12 flex justify-end">
                ...
                <template v-else>
                    <div class="relative">
                        <input v-model="searchUser" class="bg-gray-100 rounded-full px-4 py-1" type="text" placeholder="Search user">
                        <div class="grid grid-cols-1 divide-y absolute top-8 left-0 bg-white rounded-md border min-w-[140px]" v-if="foundUsers?.length">
                            <div class="px-5 py-2 flex flex-row gap-2" v-for="user in foundUsers" v-bind:key="user.id">
                                <RouterLink :to="'/profile/' + user.id">{{ user.name }}</RouterLink>
                            </div>
                        </div>
                    </div>
